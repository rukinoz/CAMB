    ...
    type EvolutionVars
        ...
        !*****RU*****
        integer ix_vcdm !Index of IDE v_dm equation (0 = inactive)
        !*****RU*****
    ...
    !IDE: v_dm
     if (EV%ix_vcdm > 0 .and. EVOut%ix_vcdm > 0) &
        yout(EVOut%ix_vcdm) = y(EV%ix_vcdm)
    !*****RU*****
    ...
    if (CP%Scalar_initial_condition/= initial_adiabatic) then
        ...
        !quintessence isocurvature modex
    end if
    ...
    !*****RU*****
    ! IDE: v_dm initial condition
    if (EV%ix_vcdm > 0) y(EV%ix_vcdm) = 0._dl
    !*****RU*****
    ...
    real(dl) :: xi_ide, f_ide, vdm_now, delta_de
    ...
    !*****RU*****
    ! Conventional IDE (Model A): modified DM background in the perturbation equations,
    ! consistent with the IDE matter-radiation equality used in results.f90.
    !   rho_dm(a) = rho_dm0 a^-3
    !      + [xi rho_de0/(3w+xi)] * (a^-3 - a^-(3(1+w)+xi))
    ! In grho_t units (8*pi*G*rho*a^2):
    !   standard piece = grhoc/a
    !   extra piece = (xi/(3w+xi))*grhov*(a^-1 - a^-(1+3w+xi)).
    block
        real(dl) :: xi_ide_c, w_de_c, denom_c, grhoc_extra
        xi_ide_c = 0._dl
        w_de_c   = -1._dl
        ...
        if (abs(xi_ide_c) > 1.e-8_dl) then
            ...
            if (abs(denom_c) > 1.e-8_dl) then
                grhoc_extra = (xi_ide_c/denom_c) * State%grhov * &
                            ( a**(-1._dl) - a**(-(1._dl + 3._dl*w_de_c + xi_ide_c)) )
            end if
        end if
        grhoc_t = State%grhoc/a + grhoc_extra
    end block
    !*****RU*****
    ...
    !*****RU*****
    ! IDE: DM coupling (Q = xi*H*rho_de)
    if (EV%ix_vcdm > 0) then
        xi_ide = 0._dl
        select type(DE => State%CP%DarkEnergy)
        class is (TDarkEnergyEqnOfState)
            xi_ide = DE%xi
        end select
        f_ide   = xi_ide * adotoa * grhov_t / grhoc_t
        vdm_now = ay(EV%ix_vcdm)
        delta_de = dgrho_de / grhov_t
        ayprime(ix_clxc) = ayprime(ix_clxc) - f_ide * (delta_de - clxc)
        ayprime(EV%ix_vcdm) = -adotoa * vdm_now
    end if
    !*****RU*****
