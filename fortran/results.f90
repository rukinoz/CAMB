...
10      if (this%CP%Omnuh2 < 1.e-7_dl) this%CP%Omnuh2 = 0
        ...
        ! IDE correction: for interacting DM, rho_dm does not scale purely as a^-3.
        ! Therefore z_eq must be obtained from rho_b(a)+rho_dm_IDE(a)=rho_r(a),
        ! rather than from the standard present-day density ratio.
        !*****RU***** IDE correction to reported matter-radiation equality redshift
block
    real(dl) :: xi_ide_c, w_de_c, denom_c
    real(dl) :: grhor_eq
    real(dl) :: a_lo, a_hi, a_mid
    real(dl) :: f_lo, f_mid
    real(dl) :: grhoc_extra_lo, grhoc_extra_mid
    integer :: i

    xi_ide_c = 0._dl
    w_de_c   = -1._dl

    select type(DE => this%CP%DarkEnergy)
    class is (TDarkEnergyEqnOfState)
        xi_ide_c = DE%xi
        w_de_c   = DE%w_lam
    end select

    if (abs(xi_ide_c) > 1.e-8_dl) then

        grhor_eq = this%grhog + this%grhornomass + &
            sum(this%grhormass(1:this%CP%Nu_mass_eigenstates))

        denom_c = 3._dl*w_de_c + xi_ide_c

        if (abs(denom_c) > 1.e-8_dl) then

            a_lo = 1.e-8_dl
            a_hi = 1.e-2_dl

            grhoc_extra_lo = (xi_ide_c/denom_c) * this%grhov * &
                (a_lo**(-1._dl) - a_lo**(-(1._dl + 3._dl*w_de_c + xi_ide_c)))

            f_lo = this%grhob/a_lo + this%grhoc/a_lo + grhoc_extra_lo - grhor_eq/a_lo**2

            do i = 1, 200
                a_mid = 0.5_dl*(a_lo + a_hi)

                grhoc_extra_mid = (xi_ide_c/denom_c) * this%grhov * &
                    (a_mid**(-1._dl) - a_mid**(-(1._dl + 3._dl*w_de_c + xi_ide_c)))

                f_mid = this%grhob/a_mid + this%grhoc/a_mid + &
                    grhoc_extra_mid - grhor_eq/a_mid**2

                if (f_lo*f_mid <= 0._dl) then
                    a_hi = a_mid
                else
                    a_lo = a_mid
                    f_lo = f_mid
                ...
                if (abs(a_hi - a_lo) < 1.e-12_dl*a_mid) exit
            end do

            this%z_eq = 1._dl/a_mid - 1._dl
...
!*****RU*****
    ...
    do i=1, n
        ...
        !*****RU*****
        ! Conventional IDE (Model A): modified DM density for reporting,
        ! consistent with grho_no_de. densities are 8*pi*G*rho*a^4.
        ! standard piece = grhoc*a; extra piece = (xi/(3w+xi))*grhov*(a-a^(1-3w+xi)).
        block
            real(dl) :: xi_d, w_d, den_d, dm_extra
            xi_d = 0._dl; w_d = -1._dl
            select type(DE => this%CP%DarkEnergy)
            class is (TDarkEnergyEqnOfState)
                xi_d = DE%xi; w_d = DE%w_lam
            end select
            dm_extra = 0._dl
            if (abs(xi_d) > 1.e-8_dl) then
                den_d = 3._dl*w_d + xi_d
                if (abs(den_d) > 1.e-8_dl) then
                    dm_extra = (xi_d/den_d) * this%grhov * &
                        (a - a**(1._dl - 3._dl*w_d - xi_d) )
                end if
            end if
            densities(3,i) = this%grhoc * a + dm_extra
        end block
        !*****RU*****
    ...
    integer nu_i
    !*****RU*****
    ! Conventional IDE (Model A): the DM background is genuinely modified by the
    ! interaction Q = xi*H*rho_de. The DM continuity equation
    !     rho_dm' + 3H rho_dm = +xi H rho_de
    ! has the solution
    !     rho_dm(a) = rho_dm0 a^-3 + [xi rho_de0/(3w+xi)] (a^-(3(1+w)+xi) - a^-3)
    ! Checks: at a=1 the bracket -> 0 (rho_dm = rho_dm0, flatness preserved);
    !         at xi=0 the extra term -> 0 (standard a^-3, recovers wCDM/LCDM).
    ! In grho units (8*pi*G*rho*a^4): standard piece = grhoc*a; extra piece =
    !     (xi/(3w+xi)) * grhov * (a-a^(1-3w+xi)).
    real(dl) :: xi_ide, w_de, denom, grho_dm_extra
    !*****RU*****

    !*****RU***** standard DM term (grhoc*a) is added below together with the IDE correction
    grhoa2 = this%grhok * a**2 + this%grhob * a + this%grhog + this%grhornomass

    ! --- modified dark matter (Model A) ---
    xi_ide = 0._dl
    w_de   = -1._dl
    select type(DE => this%CP%DarkEnergy)
    class is (TDarkEnergyEqnOfState)
    ...
    grho_dm_extra = 0._dl
    if (abs(xi_ide) > 1.e-8_dl) then
        denom = 3._dl*w_de + xi_ide
        if (abs(denom) > 1.e-8_dl) then
            grho_dm_extra = (xi_ide/denom) * this%grhov * &
                (a - a**(1._dl - 3._dl*w_de - xi_ide) )
        end if
    end if

    grhoa2 = grhoa2 + this%grhoc * a + grho_dm_extra
    !*****RU*****
