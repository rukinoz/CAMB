    ...
    contains
        ...
        procedure :: PerturbedStressEnergy
    ...
    type, extends(TDarkEnergyModel) :: TDarkEnergyEqnOfState
        ...
        !*****RU*****
        real(dl) :: xi = 0._dl  ! IDE coupling parameter (Q = xi*H*rho_de)
        !*****RU*****
    ...
    if(.not. this%use_tabulated_w) then
        grho_de = a ** (1._dl - 3. * this%w_lam - 3. * this%wa - this%xi)   ! *****RU***** IDE: rho_de ~ a^(-(3(1+w)+xi))
    ...
    this%is_cosmological_constant = .not. this%use_tabulated_w .and. &
        abs(this%w_lam + 1._dl) < 1.e-6_dl .and. &
        this%wa == 0._dl .and. &
        abs(this%xi) < 1.e-12_dl
